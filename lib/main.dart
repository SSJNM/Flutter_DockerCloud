              ...
              child: Card(
                ...
                child: InkWell(
                  ...
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => NodeIPRoute()),
                    );
                  },
                  child: Container(
                    width: 300,
...
web(img_name, {cont_name, ports, net_name, sv_path, dv_path}) async {
  if (cont_name == null &&
      ...
      sv_path == null &&
      dv_path == null) {
    var ip = "http://${nodeip}/cgi-bin/flutter/1.py?img_name=${img_name}";
    url = ip;
  } else if (ports == null &&
      net_name == null &&
      sv_path == null &&
      dv_path == null) {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/3.py?img_name=${img_name}&cont_name=${cont_name}";
    url = ip;
  } else if (cont_name == null && ports == null && net_name == null) {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/2.py?img_name=${img_name}&sv_path=${sv_path}&dv_path=${dv_path}";
    url = ip;
      ...
      dv_path == null) {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/4.py?img_name=${img_name}&ports=${ports}";
    url = ip;
  } else if (cont_name == null &&
      ports == null &&
      sv_path == null &&
      dv_path == null) {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/5.py?img_name=${img_name}&net_name=${net_name}";
    url = ip;
  } else if (cont_name == null && ports == null) {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/6.py?img_name=${img_name}&net_name=${net_name}&sv_path=${sv_path}&dv_path=${dv_path}";
    url = ip;
  } else if (cont_name == null && net_name == null) {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/7.py?img_name=${img_name}&ports=${ports}&sv_path=${sv_path}&dv_path=${dv_path}";
    url = ip;
  } else if (cont_name == null && sv_path == null && dv_path == null) {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/8.py?img_name=${img_name}&net_name=${net_name}&ports=${ports}";
    url = ip;
  } else if (net_name == null && ports == null) {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/9.py?img_name=${img_name}&cont_name=${cont_name}&sv_path=${sv_path}&dv_path=${dv_path}";
    url = ip;
  } else if (ports == null && sv_path == null && dv_path == null) {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/10.py?img_name=${img_name}&cont_name=${cont_name}&net_name=${net_name}";
    url = ip;
  } else if (net_name == null && sv_path == null && dv_path == null) {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/11.py?img_name=${img_name}&cont_name=${cont_name}&ports=${ports}";
    url = ip;
  } else if (cont_name == null) {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/12.py?img_name=${img_name}&net_name=${net_name}&ports=${ports}&sv_path=${sv_path}&dv_path=${dv_path}";
    url = ip;
  } else if (ports == null) {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/13.py?img_name=${img_name}&cont_name=${cont_name}&net_name=${net_name}&sv_path=${sv_path}&dv_path=${dv_path}";
    url = ip;
  } else if (net_name == null) {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/14.py?img_name=${img_name}&cont_name=${cont_name}&ports=${ports}&sv_path=${sv_path}&dv_path=${dv_path}";
    url = ip;
  } else if (sv_path == null && dv_path == null) {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/15.py?img_name=${img_name}&cont_name=${cont_name}&net_name=${net_name}&ports=${ports}";
    url = ip;
  } else {
    var ip =
        "http://${nodeip}/cgi-bin/flutter/16.py?img_name=${img_name}&cont_name=${cont_name}&net_name=${net_name}&ports=${ports}&sv_path=${sv_path}&dv_path=${dv_path}";
    url = ip;
  }
...
class _ContaineralllistState extends State<Containeralllist> {
  ...
  mylist() async {
    var list = await http.get("http://${nodeip}/cgi-bin/cont/all.py");
    setState(() {
      listbody = list.body;
    });
    return listbody;
...
class _ContainerlistState extends State<Containerlist> {
  var listbody = " Tap on the VIEW button";
  mylist() async {
    var list = await http.get("http://${nodeip}/cgi-bin/cont/list.py");
    setState(() {
      listbody = list.body;
    });
    return listbody;
...
class Startcont extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    rmall(rmcont) async {
      var all = await http
          .get("http://${nodeip}/cgi-bin/cont/start.py?str_cont=${rmcont}");
      contrm = all.body;
      return contrm;
    }
...
class Stopcont extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    rmall(rmcont) async {
      var all = await http
          .get("http://${nodeip}/cgi-bin/cont/stop.py?stp_cont=${rmcont}");
      contrm = all.body;
      return contrm;
    }
...
class Reomvecont extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    rmall(rmcont) async {
      var all = await http
          .get("http://${nodeip}/cgi-bin/cont/remove.py?rm_cont=${rmcont}");
      contrm = all.body;
      return contrm;
    }
  ...
  Widget build(BuildContext context) {
    ...
    rmall(rmcont, imgn) async {
      var all = await http.get(
          "http://${nodeip}/cgi-bin/cont/commit.py?cont_name=${rmcont}&img_name=${imgn}");
      contrm = all.body;
      return contrm;
    }
...
class _InspectcontState extends State<Inspectcont> {
  ...
  mylist(netname) async {
    var list = await http
        .get("http://${nodeip}/cgi-bin/cont/inspect.py?cont_name=${netname}");
    setState(() {
      listbody = list.body;
    });
    return listbody;
...
class _CommandcontState extends State<Commandcont> {
  ...
  mylist(netname, cmd) async {
    var list = await http.get(
        "http://${nodeip}/cgi-bin/cont/command.py?cmd=${cmd}&cont_name=${netname}");
    setState(() {
      listbody = list.body;
    });
    return listbody;
...
class _ImagelistState extends State<Imagelist> {
  var listbody = " Tap on the VIEW button";
  mylist() async {
    var list = await http.get("http://${nodeip}/cgi-bin/image/list.py");
    setState(() {
      listbody = list.body;
    });
    return listbody;
...
class RemoveImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    dimg(imgname) async {
      var all = await http
          .get("http://${nodeip}/cgi-bin/image/remove.py?rm_name=${imgname}");
      imgn = all.body;
      return imgn;
    }
...
class DownloadImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    dimg(imgname) async {
      var all = await http
          .get("http://${nodeip}/cgi-bin/image/down.py?down_img=${imgname}");
      imgn = all.body;
      return imgn;
    }
...
class _InspectimageState extends State<Inspectimage> {
  ...
  mylist(netname) async {
    var list = await http
        .get("http://${nodeip}/cgi-bin/image/inspect.py?img_name=${netname}");
    setState(() {
      listbody = list.body;
    });
    return listbody;
...
class _NetworkcheckState extends State<Networkcheck> {
  var listbody = " Tap on the VIEW button";
  mylist() async {
    var list = await http.get("http://${nodeip}/cgi-bin/network/list.py");
    setState(() {
      listbody = list.body;
    });
    return listbody;
...
class Networkcreate extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    dimg(imgname) async {
      var all = await http
          .get("http://${nodeip}/cgi-bin/network/create.py?new_net=${imgname}");
      imgn = all.body;
      return imgn;
    }
...
class Networkremove extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    dimg(imgname) async {
      var all = await http
          .get("http://${nodeip}/cgi-bin/network/remove.py?rm_net=${imgname}");
      imgn = all.body;
      return imgn;
    }
...
class Networkconnect extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    dimg(imgname, contname) async {
      var all = await http.get(
          "http://${nodeip}/cgi-bin/network/connect.py?new_net=${imgname}&cont_name=${contname}");
      netn = all.body;
      return netn;
    }
...
class Networkdisconnect extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    dimg(imgname, contname) async {
      var all = await http.get(
          "http://${nodeip}/cgi-bin/network/disconnect.py?new_net=${imgname}&cont_name=${contname}");
      netn = all.body;
      return netn;
    }
...
class _InspectnetworkState extends State<Inspectnetwork> {
  ...
  mylist(netname) async {
    var list = await http
        .get("http://${nodeip}/cgi-bin/network/inspect.py?net_name=${netname}");
    setState(() {
      listbody = list.body;
    });
    return listbody;
...
class _VolumecheckState extends State<Volumecheck> {
  var listbody = " Tap on the VIEW button";
  mylist() async {
    var list = await http.get("http://${nodeip}/cgi-bin/volume/list.py");
    setState(() {
      listbody = list.body;
    });
    return listbody;
...
class Volumecreate extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    dimg(imgname) async {
      var all = await http
          .get("http://${nodeip}/cgi-bin/volume/create.py?new_vol=${imgname}");
      imgn = all.body;
      return imgn;
    }
...
class Volumeremove extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    dimg(imgname) async {
      var all = await http
          .get("http://${nodeip}/cgi-bin/volume/remove.py?rm_vol=${imgname}");
      imgn = all.body;
      return imgn;
    }
...
class _VolumeinspectState extends State<Volumeinspect> {
  ...
  mylist(netname) async {
    var list = await http
        .get("http://${nodeip}/cgi-bin/volume/inspect.py?vol_name=${netname}");
    setState(() {
      listbody = list.body;
    });
    return listbody;
...
//Last Route from MyHome

class NodeIPRoute extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    var mybody = Container(
      height: double.infinity,
